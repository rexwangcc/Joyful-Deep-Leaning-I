...
BH_TSNE_BIN_PATH = (
    path_join(dirname(__file__), "windows", "bh_tsne.exe")
    if IS_WINDOWS
    else path_join(dirname(__file__), "bh_tsne")
)
assert isfile(BH_TSNE_BIN_PATH), (
    "Unable to find the bh_tsne binary in the "
...
def init_bh_tsne(
    samples,
    workdir,
    no_dims=DEFAULT_NO_DIMS,
    initial_dims=INITIAL_DIMENSIONS,
    perplexity=DEFAULT_PERPLEXITY,
    theta=DEFAULT_THETA,
    randseed=EMPTY_SEED,
    verbose=False,
    use_pca=DEFAULT_USE_PCA,
    ...
    with open(path_join(workdir, "data.dat"), "wb") as data_file:
        ...
        data_file.write(
            pack(
                "iiddii", sample_count, sample_dim, theta, perplexity, no_dims, max_iter
            )
        )
        # Then write the data
...
def bh_tsne(workdir, verbose=False):
    ...
    with open(path_join(workdir, "result.dat"), "rb") as output_file:
        ...
        results = [
            _read_unpack("{}d".format(result_dims), output_file)
...
def main(args):
    ...
    for result in run_bh_tsne(
        argp.input,
        no_dims=argp.no_dims,
